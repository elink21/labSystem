...
clock = setInterval(function () {
  ...
  c.innerHTML = d.toLocaleString("en-US", {
    hour: "numeric",
    minute: "numeric",
    hour12: true,
  });
...
function updateAuditTable(toSearch) {
  ...
  for (let item of items) {
    if (
      item["patrimonialNumber"].includes(toSearch) ||
...
function updatePieChart() {
  ...
  for (let lending of historialLendings) {
    let field = searchByField(
      students,
      "accountNumber",
      lending["accountNumber"]
    );
    let career = null;
    if (field[0] != undefined) {
      career = field[0]["career"];
    }
    if (career != null)
...
function fillActiveLendings() {
  ...
  for (lending of lendings) {
    let studentName = searchByField(
      students,
      "accountNumber",
      lending["accountNumber"]
    )[0]["name"];
    let studentAccount = searchByField(
      students,
      "accountNumber",
      lending["accountNumber"]
    )[0]["accountNumber"];
    let itemName = searchByField(
      items,
      "patrimonialNumber",
      lending["patrimonialNumber"]
    )[0]["name"];
    let card = `
